...
WITH mid, sum($mids[i.amount_bucket]) AS in_amt, count(i) AS in_cnt, collect(DISTINCT src) AS srcs, min(i.timestamp) as min_i, max(i.timestamp) as max_i  # noqa: E501
...
WITH mid, in_amt, in_cnt, srcs, min_i, max_i,
     sum($mids[o.amount_bucket]) AS out_amt, count(o) AS out_cnt, collect(DISTINCT dst) AS dsts, min(o.timestamp) as min_o, max(o.timestamp) as max_o  # noqa: E501
WHERE in_cnt >= $min_txns AND out_cnt >= $min_txns AND in_amt > 0
  AND abs(out_amt - in_amt) <= $tolerance * in_amt
WITH [mid] + srcs + dsts AS ns, mid, in_amt, out_amt, in_cnt + out_cnt AS fan, CASE WHEN min_i < min_o THEN min_i ELSE min_o END as min_ts, CASE WHEN max_i > max_o THEN max_i ELSE max_o END as max_ts  # noqa: E501
